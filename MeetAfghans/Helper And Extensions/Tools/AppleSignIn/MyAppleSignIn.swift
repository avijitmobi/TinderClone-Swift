...
typealias AppleSignInBlock = ((_ userInfo:AppleInfoModel?,_ error:NSError?)->())?


class MyAppleSignIn : NSObject {
    
//    static let shared = MyAppleSignIn()
...
extension MyAppleSignIn:ASAuthorizationControllerPresentationContextProviding, ASAuthorizationControllerDelegate{
    ...
    func presentationAnchor(for controller: ASAuthorizationController) -> ASPresentationAnchor {
        return keyWindow?.rootViewController?.view.window ?? UIView().window!
    }
    ...
    func authorizationController(controller: ASAuthorizationController, didCompleteWithError error: Error) {
        appleSignInBlock?(nil,error as NSError)
    }
}
