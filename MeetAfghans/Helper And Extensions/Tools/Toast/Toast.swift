...
//
//  Created by sudheer-kumar on 16/04/18.
...
public struct ToastStyle {
     ...
     */
    public var activitySize = CGSize(width:50.0, height:50.0)
     ...
     */
    public var activityBackgroundColor: UIColor = CommonColor.ButtonGradientFirst
...
public enum ToastPosition {
    ...
    fileprivate func centerPoint(forToast toast: UIView, inSuperview superview: UIView) -> CGPoint {
        ...
        case .center:
            return CGPoint(x: superview.bounds.size.width / 2.0, y: superview.bounds.size.height / 2)
        case .bottom:
            return CGPoint(x: superview.bounds.size.width / 2.0, y: (superview.bounds.size.height - (toast.frame.size.height / 2.0)) - bottomPadding - 50) // deducted 30
        }
    }
}
